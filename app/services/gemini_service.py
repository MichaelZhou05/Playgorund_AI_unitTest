...
def generate_answer_with_history(
    ...
    """
    Generates a conversational answer using RAG context and conversation history.
    
    This function:
    1. Retrieves context from RAG service based on latest user message
    2. Constructs prompt with full conversation history and context
    3. Generates contextually-aware answer
    4. Returns answer with source citations
    
    Args:
        corpus_id: RAG corpus resource name
    ...
    Example:
        ...
        answer, sources = generate_answer_with_history(corpus_id, history)
    """
    if not project_id:
        raise ValueError("GOOGLE_CLOUD_PROJECT environment variable not set")
    ...
    try:
        # Import RAG service to retrieve context
        from app.services.rag_service import retrieve_context_with_history
        
        logger.info(f"Generating conversational answer with history ({len(history)} messages)...")
        
        # Step 1: Retrieve context considering conversation history
        context_texts, source_names, last_user_message = retrieve_context_with_history(
            corpus_id, history, top_k, threshold
        )
        ...
        # Step 2: Construct prompt with conversation history and context
        combined_context = "\n\n".join(context_texts)
...
4. Maintain a natural conversational flow from previous messages
5. If you need to refer back to previous answers, do so naturally
6. If the materials don't contain enough information, say so
7. Use a friendly, professional teaching tone
...
Answer:"""
        ...
        logger.info(f"Generated conversational answer with {len(source_names)} citations")
        
        return (answer_text, source_names)
        
    except Exception as e:
        logger.error(f"Failed to generate conversational answer: {str(e)}")
        raise
